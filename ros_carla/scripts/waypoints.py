...
class Creating_Path:
    ...
    def __init__(self,vehicle):
        ...
        self.length=0
        self.lock_of_thread=threading.Lock()
    ...
    def making_graph_from_crossroad(self):
        # for i in list(self.graph.nodes()): TOWA E ORIGINALA
        for i in list(self.graph.nodes):
    ...
    def choose_random(self):
        # return list(self.graph.nodes)[10]
    ...
    # towa e nowo po slu4aya na boba
    ...
    def create_path_to_closest_crossroad(self):
        ...
        self.waypoints_from_the_path=remain_waypoints

        self.lock_of_thread.release()

    def create_path(self):
        # if len(self.waypoints_from_the_path) == 0:
        #     close_one_wp_to_car=self.get_closest_wp_to_the_car()
        #     waypoints_to_crossroad=self.get_wp_to_crossroad(close_one_wp_to_car)
        #     self.waypoints_from_the_path=waypoints_to_crossroad    

        # self.path=nx.shortest_path(self.graph,self.getting_closest_one_wp_from_car(self.waypoints_from_the_path[-1].transform),self.choose_random())

        self.lock_of_thread.acquire()

        random=self.choose_random()
        self.path=self.graph.dijkistra(self.getting_closest_one_wp_from_car(self.waypoints_from_the_path[-1].transform),random)
        # print random
        ...
        # self.path=self.graph.dijkistra(self.getting_closest_one_wp_from_car(remain_waypoints[-1].transform),self.choose_random())
        # # waypoints= self.converting_the_path_to_list_of_waypoints(self.getting_all_waypoints_from_node_to_node())
        # # self.waypoints_from_the_path=remain_waypoints+waypoints