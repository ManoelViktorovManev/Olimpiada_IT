...
class MyRadar:
            ...
            if distance > distance_actor_and_point:
                ...
                if i.type_id.find("speed_limit")>=0:
                    if self.get_if_one_speed_sign_is_in_road(i) is True:
                        list.append(i)
                    elif self.get_if_one_speed_sign_is_in_road_using_wp(i,list_of_wp) is True:
                        list.append(i)
    ...
    def get_if_one_speed_sign_is_in_road_using_wp(self,actor,list_of_wp):
        for i in self.dict:
            if i.id is actor.id:
                waypoint=self.dict.get(i)
                break
        list_of_road_id=[]
        for x in list_of_wp:
            if x.road_id not in list_of_road_id:
                list_of_wp.append(x.road_id)
        if actor.road_id in list_of_road_id:
            return True
        return False
